...
test('keyboard Trainer renders both child components', () => {
    ...
    expect(
        screen.getByRole('heading', { name: 'Standard QWERTY' })
    ).toBeInTheDocument(); // From KeyboardViewer
    expect(
        screen.getByRole('heading', { name: 'Typing Area' })
    ).toBeInTheDocument(); // From KeyboardText
});
...
test('keyboard Trainer shows description', () => {
    ...
    const description = screen.getByText(
        /Learn touch typing with an interactive keyboard layout visualizer/
    );
