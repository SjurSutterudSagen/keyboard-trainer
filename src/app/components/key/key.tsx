...
export function Key({
    ...
    if (keymap && keymap.mapping[keyDef.code]) {
        ...
        displayKey = {
            ...keyDef,
            key:
                isShiftPressed && mapping.shiftKey
                    ? mapping.shiftKey
                    : mapping.key,
            label:
                isShiftPressed && mapping.shiftLabel
                    ? mapping.shiftLabel
                    : mapping.label,
        };
    }
    ...
    const baseClasses =
        'absolute border-2 rounded-md transition-all duration-75 flex items-center justify-center text-sm font-medium select-none';
    ...
    const modifierClasses = keyDef.isModifier
        ? 'bg-blue-50 border-blue-200'
        : '';
    const alphanumericClasses = keyDef.isAlphanumeric ? 'font-bold' : '';
    const zoneClass = keyDef.zone
        ? zoneClasses[keyDef.zone] || zoneClasses.center
        : zoneClasses.center;
        ...
        <div
            ...
            style={{
                ...
                transform: keyDef.position.rotation
                    ...
                    : undefined,
                transformOrigin:
                    keyDef.position.rotationX && keyDef.position.rotationY
                        ? `${keyDef.position.rotationX}px ${keyDef.position.rotationY}px`
                        : 'center',
            }}
            title={`${displayKey.label} (${keyDef.code})`}
        >
            <span className="text-xs truncate px-1">{displayKey.label}</span>
        </div>
    );
}
